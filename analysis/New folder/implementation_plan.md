...
> [!NOTE]
> **Similarity Threshold**: We will use a hardcoded value of 0.65 in the `address.py` code. This means you do not need to add anything to your `.env` file right now. This threshold provides a good balance between allowing STT errors and preventing incorrect matches.

> [!NOTE]
> **House Number Handling**: As agreed, we will verify ONLY the street name and postal code for deliverability. The system will still collect the house number for the driver, but the exact house number won't be required to match the postal code data file. This prevents false negatives where the STT hears the wrong house number.
